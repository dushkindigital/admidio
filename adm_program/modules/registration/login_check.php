...
else
{
    ...
    else
    {
        ...
        # first login
        if (\intval($gCurrentUser->getValue('usr_number_login')) <= 1) {
        	admRedirect(ADMIDIO_URL . '/adm_custom/password_change.php?mode=html&usr_id=' . $gCurrentUser->getValue('usr_id'));
        	// => EXIT
        }

        # expired temp password
        if (\strcasecmp($gCurrentUser->getValue('L4P_DB_TEMP_PASS_EXPIRATION'), \date('Y-m-d')) < 0) {
        	admRedirect(ADMIDIO_URL . '/adm_custom/handle_password_expiry.php');
        	// => EXIT
        }
