...
function handle_form_post ( $organisation_id ) {
    ...
    // pluck out private data and store in $privateData : Starts here
    $privateData = [];
    
    foreach ($GLOBALS['gProfileFields']->mProfileFields as $key => $field) {
        if(in_array($field->getValue('usf_name_intern'), $privateColumns)) {    
            ...
            unset($GLOBALS['gProfileFields']->mProfileFields[$key]);
        } 
    }
    // pluck out private data and store in $privateData : Ends here
    ...
    try {
        ...
        $savedUser = $user->save();
        $getLastInsertId = "SELECT max(usr_id) AS usr_id FROM ".TBL_USERS;
        $insertedQuery = $GLOBALS['gDb']->query($getLastInsertId);
        $insertedId = $insertedQuery->fetch();
        
        $userId = $insertedId['usr_id'];
    
        $privateFormData = [];

        foreach ($privateData as $key => $value) {
            $privateFormData[strtolower($key)] = "'$value'";
        }
        $privateFormData['uapp_usr_id'] = $userId;

        $privateFormKeys = implode(', ', array_keys($privateFormData));
        $privateFormValues = implode(', ', $privateFormData);
        $tableApplicationQuery = "INSERT INTO ".TBL_APPLICATIONS."
                                    ( $privateFormKeys )
                                    VALUES (
                                        $privateFormValues
                                    )";
        $privateDataSaved = $GLOBALS['gDb']->query($tableApplicationQuery);
